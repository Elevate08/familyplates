...
It ships as two editions from one codebase: the self-hosted **appliance**, and the **hosted service**, which adds a `saas/` Rails engine for billing, sign-up, support and the operator console. See [Editions](editions.md) for what each includes and where the code meets.
...
* **Multi-Tenant Hosted Mode & Tenant Isolation:** Phase 4 splits `FamilyPlates.installed?` from individual tenant presence (`Current.household.present?`), allowing FamilyPlates to run either as a self-hosted single-family appliance (`FAMILYPLATES_MODE=appliance`) or as the multi-tenant hosted service (`FAMILYPLATES_MODE=hosted`, which needs the `saas/` engine; see [Editions](editions.md)). In hosted mode, tenant boundaries are enforced before user identity: unauthenticated requests are directed to sign in or sign up, and new organizers can register kitchens via `/signup` with single-use 6-character email verification codes. Onboarding (`households.onboarded_at`) runs independently per household, seeding starter recipes and pantry staples into that tenant's records. Active profile cookies and all model associations are scoped strictly to the authenticated tenant's database rows, verified by a comprehensive cross-tenant test suite that passes with zero exemptions.
* **Subscriptions, Billing & Entitlements:** Phase 5 introduces a hosted subscription and billing layer powered by the `Pay` gem (`Household::Billing` in the `saas/` engine, which declares `pay_customer`), pointed at Stripe. Appliance mode deployments remain 100% free and permanently entitled. In hosted mode, households receive a 14-day free trial upon registration. Active subscriptions ($4/month or $35/year with a 27% discount) grant full kitchen access. When payments fail, a 7-day grace period ensures families are not abruptly locked out of dinner planning while updating payment details. Canceled subscriptions maintain access through the end of the current billing cycle. Even if a subscription expires, household data (recipes, meal history, pantry staples) is permanently preserved without deletion.
